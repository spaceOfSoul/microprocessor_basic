...
ISR(INT3_vect){ // SW3
	if(dutyCycle_value > 0) { // 감소
		dutyCycle_value-=10;
	}
	set_timer3_dutycycle(dutyCycle_value);
	fnd_write_numbers((uint16_t)dutyCycle_value); // fnd에 숫자 쓰기
}
ISR(INT7_vect){ // SW2
	if(dutyCycle_value < 100) { // 증가
		dutyCycle_value+=10;
	}
	set_timer3_dutycycle(dutyCycle_value);
	fnd_write_numbers((uint16_t)dutyCycle_value);
}
